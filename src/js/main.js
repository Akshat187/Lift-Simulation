...
document.querySelector("#submit").addEventListener("click", (event) => {
    console.log("The event listener is ", event)
    ...
    console.log("The lift count is ", floorsCount)
...
function handleLiftCall(event) {
    ...
    const calledFloor = event ? path[2] : 0
    // const calledFloor = 0;
...
function moveLift(floorId, liftId) {
    console.log("hi i am", floorId);
    console.log("akshat", liftId);
    if (floorLiftMap.get(floorId) != null) {
        const mappedLiftId = floorLiftMap.get(floorId);
        if (liftsAvailabilitiy.get(mappedLiftId)) {
            liftsAvailabilitiy.set(mappedLiftId, false);
    ...
    console.log("myfloor id is", floorId)
...
function renderFloors(totalFloors) {
    ...
    for (let floorNumber = totalFloors; floorNumber > 0; floorNumber--) {
        ...
        `;
        currentFloor.querySelector(".lift-control-up").addEventListener("click", (event) => handleLiftCall(event));
        currentFloor.querySelector(".lift-control-down").addEventListener("click", (event) => handleLiftCall(event));
    ...
    `;
    groundFloor.querySelector(".lift-control-up").addEventListener("click", (event) => handleLiftCall(event));
